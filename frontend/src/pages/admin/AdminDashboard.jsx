        ...
        <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-3">
                    <div className="px-4 py-2 bg-white border border-slate-200 rounded-lg text-xs font-semibold text-slate-600 flex items-center gap-2 shadow-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6">
                {[
                    ...
                    {
                        label: 'Total Platform Yield',
                        value: `$${parseFloat(stats.totalEarnings || 0).toLocaleString()}`,
                        icon: DollarSign,
                        color: 'indigo'
                    }
                ].map((card, i) => (
                    <div key={i} className="bg-white p-8 rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition-all relative overflow-hidden group">
                    ...
                    <div className="space-y-6">
                        {stats.recentUsers?.slice(0, 6).map((user, i) => (
                            <div key={i} className="flex items-center justify-between group cursor-pointer hover:translate-x-1 transition-transform">
                                <div className="flex items-center gap-3">
                                    <div className="w-10 h-10 rounded-full bg-slate-100 border border-white shadow-sm overflow-hidden">
                                        <img src={`https://ui-avatars.com/api/?name=${user.first_name || 'U'}&background=EEF2FF&color=4F46E5&bold=true`} alt="Avatar" className="w-full h-full object-cover" />
                                    </div>
                                    <div className="flex flex-col">
                                        <span className="text-[13px] font-semibold text-slate-900 group-hover:text-indigo-600 transition-colors">{user.first_name || 'Anonymous'}</span>
                                        <span className="text-[11px] text-slate-400 font-medium">Joined 2h ago</span>
                                    </div>
                                </div>
                ...
                <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm flex items-center gap-8">
                    ...
                    <div className="space-y-1">
                        ...
                        <div className="flex items-baseline gap-2">
                            <span className="text-3xl font-bold text-indigo-600 tracking-tight">99.8%</span>
                            <span className="text-[10px] font-bold text-emerald-500 uppercase tracking-widest">Uptime</span>
                        </div>
                        <p className="text-xs font-medium text-slate-400">All systems operational in US-East-1</p>
                    </div>
                </div>
            </div>
        </div>
